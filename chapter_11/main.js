...
async function locateScalpel(nest) {
  let currentHop = nest.name;
  let nextHop = await anyStorage(nest, currentHop, "scalpel");
  while(nextHop !== currentHop) {
    currentHop = nextHop;
    nextHop = await anyStorage(nest, currentHop, "scalpel");
  }
  return currentHop;
}

function locateScalpel2(nest) {
  function checkCurrent(currentLocation) {
    return anyStorage(nest, currentLocation, "scalpel").then(nextLocation => {
      if (nextLocation == currentLocation) {
        return currentLocation;
      } else {
        return checkCurrent(nextLocation);
      }
    });
  }

  return checkCurrent(nest.name);
}
